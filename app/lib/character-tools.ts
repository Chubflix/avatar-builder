...
import {ToolRunnableConfig} from "@langchain/core/dist/tools/types";

/**
 * Context schema for tool runtime
 */
export const contextSchema = z.object({
    supabase: z.instanceof(SupabaseClient), // SupabaseClient (any for complex objects)
...
export function createCharacterTools() {
    ...
    const getGreetingsCountTool = tool(
        async (_, {context}: {context: ContextType}) => {
    ...
    const deleteStoryPhasesTool = tool(
        ...
        },
        {
            ...
            schema: z.object({phaseIds: z.array(z.string()).min(1)}),
        }
    );

    // Story Phases: assign many greeting
    const assignStoryPhaseTool = tool(
        async (
            {
                greetingIds,
                story_phase_id,
        ...
        },
        {
            ...
            description:
                'Assign multiple greetings to a story phase. Requires the greeting IDs which you can get from get_all_greetings and a phase ID which you can get from get_all_story_phases.',
            schema: z.object({
                greetingIds: z.array(z.string()).min(1).describe('The UUIDs of the greeting to update'),
